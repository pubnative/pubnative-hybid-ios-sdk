...
- (void)test_adLinear
{
    ...
    [self.parser parseWithData:vastData completion:^(HyBidVASTModel *vastModel, HyBidVASTParserError vastError) {
        ...
        XCTAssertNil([firstAdFirstCreative linear]);
        XCTAssertNotNil([firstAdSecondCreative linear]);
        XCTAssertNil([secondAdFirstCreative linear]);
        XCTAssertNotNil([secondAdSecondCreative linear]);
