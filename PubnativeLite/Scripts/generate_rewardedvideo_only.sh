...
# Generate Frameworks
xcodebuild -workspace HyBid.xcworkspace -scheme HyBidRewardedVideoOnly -sdk iphoneos -configuration Release clean build CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO CONFIGURATION_BUILD_DIR=$IPHONEOS_PATH | xcpretty -c
xcodebuild -workspace HyBid.xcworkspace -scheme HyBidRewardedVideoOnly -sdk iphonesimulator -configuration Release clean build CONFIGURATION_BUILD_DIR=$IPHONESIMULATOR_PATH | xcpretty -c
