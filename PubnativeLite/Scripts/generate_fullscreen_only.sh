...
# Generate Frameworks
xcodebuild -workspace HyBid.xcworkspace -scheme HyBidFullScreenOnly -arch arm64 -sdk iphoneos -configuration Release clean build CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO CONFIGURATION_BUILD_DIR=$IPHONEOS_PATH | xcpretty -c
xcodebuild -workspace HyBid.xcworkspace -scheme HyBidFullScreenOnly -arch x86_64 -sdk iphonesimulator -configuration Release clean build CONFIGURATION_BUILD_DIR=$IPHONESIMULATOR_PATH | xcpretty -c
