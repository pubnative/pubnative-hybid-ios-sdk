...
find "$BASE_DIR" -depth -type f -name "hybid*.js" ! -path "*/Pods/*" ! -path "*/OMSDK*" ! -path "*/PubnativeLiteDemo/*" ! -path "*/PubnativeLiteTests/*" | while read -r file; do
    dir=$(dirname "$file")
    name=$(basename "$file")
    newname=$(echo "$name" | sed -E "s/hybid/${NAMESPACE_LOWER}/g")
    if [ "$name" != "$newname" ]; then
        mv "$file" "$dir/$newname"
        echo "✅ Renamed: $name → $newname"
    fi
...
find "$BASE_DIR" -type f \( -name "*.h" -o -name "*.m" -o -name "*.mm" -o -name "*.swift" \) \
    ...
    sed -i '' -E "s/hybid_/${NAMESPACE_LOWER}_/g" "$file"
    # ObjC selectors / camelCase (e.g. hyBidNoFill → ngsdkNoFill)
    sed -i '' -E "s/hyBid/${NAMESPACE_LOWER}/g" "$file"
    # Class/product names
    sed -i '' -E "s/HyBid/${NAMESPACE}/g" "$file"
    sed -i '' -E "s/PNLite/${NAMESPACE}/g" "$file"
    sed -i '' -E "s/PubnativeLite/${NAMESPACE}/g" "$file"
    # Replace API URLs before generic pubnative→ngsdk swap to avoid corrupting domains
    ...
    sed -i '' -E "s/pubnative/${NAMESPACE_LOWER}/g" "$file"
    ...
    sed -i '' -E "s/hybidmraidscaling/${NAMESPACE_LOWER}mraidscaling/g" "$file"
    sed -i '' -E "s/hybidscaling/${NAMESPACE_LOWER}scaling/g" "$file"
    # MRAID/scaling: element id and selectors (hybid-ad → ngsdk-ad) used in .js and in ObjC HTML strings
    sed -i '' -E "s/hybid-ad/${NAMESPACE_LOWER}-ad/g" "$file"
    # OMSDK partner identifier string (so built binary does not expose "Pubnativenet")
    sed -i '' -E "s/\"Pubnativenet\"/\"${NAMESPACE}\"/g" "$file"
    # DispatchQueue label and UserDefaults key value (remove hybid references)
    sed -i '' -E "s/hybid\.network\.monitor/${NAMESPACE_LOWER}.network.monitor/g" "$file"
    sed -i '' -E "s/com\.pubnative\.hybid-ios-sdk/net.nextgen.${NAMESPACE_LOWER}/g" "$file"
...
if [ "${NAMESPACE}" = "NGSDK" ]; then
    echo "📐 Keeping only integration type Smaato in viewability..."
    if bundle exec ruby "$(dirname "$0")/namespace_omsdk_smaato_only.rb" "${BASE_DIR}"; then
...
if [ -f "$GENERATE_SCRIPT" ]; then
    ...
    bash "$GENERATE_SCRIPT" "HyBid" "${NAMESPACE}"
else
